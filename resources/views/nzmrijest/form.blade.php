...
<div class="sbox">
	...
	<div class="sbox-content"> 	
		...
		<ul class="parsley-error-list">
		 ...
		 {!! Form::open(array('url'=>'nzmrijest/save?return='.$return, 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ')) !!}
<div class="col-md-12">
				...
				<div class="input-group m-b" style="width:150px !important;">
					...
					<input type='radio' name='status' value ='open'  @if($row['status'] == 'open') checked="checked" @endif > Open </label>
					<label class='radio radio-inline'>
					<input type='radio' name='status' value ='close'  @if($row['status'] == 'close') checked="checked" @endif > Close </label> 
										 </div> 
										 <div class="col-md-1">
			...
			<div style="clear:both"></div>	
				  ...
				  <div class="form-group">
					...
					<button type="button" onclick="location.href='{{ URL::to('nzmrijest?return='.$return) }}' " class="btn btn-warning btn-sm "><i class="icon-cancel-circle2 "></i>  {{ Lang::get('core.sb_cancel') }} </button>
					</div>	  
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#id").jCombo("{!! url('nzmrijest/comboselect?filter=tb_users:id:first_name|last_name') !!}",
		{  selected_value : '{{ $row["id"] }}' });
		 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("nzmrijest/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
